...
/// Config with `blank-line-at-block-start` enabled.
fn blank_line_at_block_start_config() -> Config {
    Config {
...
fn comment_multiset_no_ws(src: &str) -> Vec<String> {
    let mut comments: Vec<String> = jals_exec::block_on_inline(Parse::parse(src))
        .syntax()
        .descendants_with_tokens()
        .filter_map(SyntaxElement::into_token)
        .filter(|t| {
...
/// Config with `switch-expression-on-new-line` enabled. Layout-only, so no width tuning is
/// needed — the break fires structurally whenever a switch expression is the value of a `=`.
fn switch_on_new_line_config() -> Config {
    Config {
...
fn import_keys(src: &str) -> Vec<(bool, String)> {
    ...
    let file = SourceFile::cast(jals_exec::block_on_inline(Parse::parse(src)).syntax())
        .expect("parse yields a source file");
    file.imports()
        .map(|imp| {
...
proptest! {
    ...
    /// Preserving a body's leading blank line stays idempotent.
    #[test]
    fn blank_line_at_block_start_idempotent(src in javaish()) {
        let once = fmt_with(&src, &blank_line_at_block_start_config());
        let twice = fmt_with(&once, &blank_line_at_block_start_config());
        prop_assert_eq!(once, twice);
    }

    /// Preserving a body's leading blank line only touches whitespace, never significant tokens.
    #[test]
    fn blank_line_at_block_start_preserves_significant_tokens(src in javaish()) {
        let out = fmt_with(&src, &blank_line_at_block_start_config());
        prop_assert_eq!(sig_tokens(&src), sig_tokens(&out));
    }

    /// Preserving a body's leading blank line never drops or mangles a comment (multiset preserved).
    #[test]
    fn blank_line_at_block_start_preserves_comments(src in javaish()) {
        let out = fmt_with(&src, &blank_line_at_block_start_config());
        prop_assert_eq!(comment_multiset_no_ws(&src), comment_multiset_no_ws(&out));
    }
    ...
    /// Breaking a switch-expression initializer onto its own line stays idempotent: re-formatting
    /// the broken layout reproduces it.
    #[test]
    fn switch_on_new_line_idempotent(src in javaish()) {
        let cfg = switch_on_new_line_config();
        let once = fmt_with(&src, &cfg);
        let twice = fmt_with(&once, &cfg);
        prop_assert_eq!(once, twice);
    }

    /// The option is layout-only: it only changes the whitespace after a `=`, so the
    /// significant-token sequence is preserved exactly.
    #[test]
    fn switch_on_new_line_preserves_significant_tokens(src in javaish()) {
        let out = fmt_with(&src, &switch_on_new_line_config());
        prop_assert_eq!(sig_tokens(&src), sig_tokens(&out));
    }

    /// The break never drops or mangles a comment.
    #[test]
    fn switch_on_new_line_preserves_comments(src in javaish()) {
        let out = fmt_with(&src, &switch_on_new_line_config());
        prop_assert_eq!(comment_contents(&src), comment_contents(&out));
    }

    /// The break never panics on arbitrary Unicode input.
    #[test]
    ...
    fn reorder_mods_emits_canonical_order(src in java_with_modifiers()) {
        let out = fmt_with(&src, &reorder_mods_config());
        for m in jals_exec::block_on_inline(Parse::parse(&out))
            .syntax()
            .descendants()
