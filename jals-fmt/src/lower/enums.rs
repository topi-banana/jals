...
use crate::doc::{Doc, concat, hardline, indent, nil, text};
use crate::lower::{
    Ctx, break_before, first_sig_token, last_sig_token, lower, lower_generic, sep, tok,
};
...
fn partition(node: &SyntaxNode) -> Option<EnumParts> {
    ...
    for el in node.children_with_tokens() {
        match &el {
            ...
            SyntaxElement::Node(n) if first_sig_token(n).is_none() => continue,
            _ => {}
        }
...
pub(crate) fn lower_enum_body(node: &SyntaxNode, ctx: &Ctx<'_>) -> Doc {
    ...
    let Some(parts) = partition(node) else {
        return lower_generic(node, ctx);
    };
    ...
    if !parts.is_empty() {
        body.push(build_inner(&parts, ctx));
    }
    if has_dangling {
        ...
        body.push(ctx.comments.dangling(rbrace));
    }
...
fn build_inner(parts: &EnumParts, ctx: &Ctx<'_>) -> Doc {
    let mut rows: Vec<Doc> = Vec::new();

    // Glue the terminator onto the last constant only when there are constants, no trailing comma
    // sits between, and the `;` carries no leading comment (which would otherwise be relocated
    // above the constant, dropping it off its anchor and breaking idempotency). Carrying the token
    // (not a bool) lets the gluing site emit it directly without re-deriving it.
    let glued_terminator: Option<&SyntaxToken> =
        if parts.constants.is_empty() || parts.has_trailing_comma() {
            ...
            parts
                .terminator
                .as_ref()
                .filter(|s| !ctx.comments.has_leading(s))
        };
    ...
    {
        if !rows.is_empty() {
            rows.push(row_sep(Some(semi), ctx));
        }
        rows.push(tok(semi, ctx));
    }
    ...
    for el in &parts.members {
        ...
        if !rows.is_empty() {
            rows.push(row_sep(first.as_ref(), ctx));
        }
        rows.push(match el {
            SyntaxElement::Node(n) => lower(n, ctx),
            SyntaxElement::Token(t) => tok(t, ctx),
        });
    }
...
/// [`crate::lower::break_before`] (the same blank-line-aware break class bodies use), but tolerates
...
/// The constant's arg list and class body recurse through [`lower`] either way. Enum-constant
...
fn lower_enum_constant(node: &SyntaxNode, ctx: &Ctx<'_>) -> Doc {
    if ctx.cfg.annotation_placement != AnnotationPlacement::Expanded {
        return lower_generic(node, ctx);
    }
    ...
    for el in node.children_with_tokens() {
        ...
        let (first, last) = match &el {
            SyntaxElement::Node(n) => (first_sig_token(n), last_sig_token(n)),
