...
impl Ctx<'_> {
    ...
    pub(crate) async fn lower_chain(&self, node: &SyntaxNode) -> Doc {
        ...
        let doc = Doc::concat(vec![
            ...
            Doc::continuation_indent(Doc::concat(wrapped)),
        ]);
        Doc::group_within(doc, self.cfg.chain_width)
    }
    ...
    fn flatten_chain(node: &SyntaxNode) -> Option<(SyntaxNode, Vec<ChainLink>)> {
        ...
        let head = loop {
            match cur.kind() {
                S::FIELD_ACCESS => {
                    let recv = Self::first_child_node(&cur)?;
