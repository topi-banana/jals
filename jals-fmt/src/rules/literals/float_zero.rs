...
impl FloatTrailingZero {
    ...
    ///
    /// Let `frac` be the digit/underscore run right after the `.` (it stops at an `e` / `E`
    /// exponent marker or an `f` / `F` / `d` / `D` suffix).
    /// [`Always`](FloatLiteralTrailingZero::Always) inserts a single `0` when `frac` is empty;
    /// [`Never`](FloatLiteralTrailingZero::Never) removes `frac` when it is non-empty and consists
    /// solely of `0`s (so `1.50` and underscore fractions like `1.0_0` are left intact). Both
    /// transforms preserve the numeric value, the suffix, and the exponent, and are idempotent in a
    /// single pass.
    fn map_float_trailing_zero(lit: &str, policy: FloatLiteralTrailingZero) -> Option<String> {
        ...
        match policy {
            ...
            FloatLiteralTrailingZero::Never
                // Non-empty integer part (so the bare `.` is never produced) and an all-zero
                // fraction.
                if dot > 0
                    && frac_end > dot + 1
                    && bytes[dot + 1..frac_end].iter().all(|&b| b == b'0') =>
            {
                // Strip the whole zero run at once (`1.0` / `1.00` → `1.`) — required for
                // idempotency.
...
impl LiteralRule for FloatTrailingZero {
    fn rewrite(&self, text: &str, _kind: S) -> Option<String> {
        Self::map_float_trailing_zero(text, self.policy)
    }
}

#[cfg(test)]
mod tests {
    use super::FloatTrailingZero;
    use crate::config::FloatLiteralTrailingZero::{self, Always, Never};

    fn map_float_trailing_zero(
        lit: &str,
        policy: FloatLiteralTrailingZero,
    ) -> Option<alloc::string::String> {
        FloatTrailingZero::map_float_trailing_zero(lit, policy)
    }
