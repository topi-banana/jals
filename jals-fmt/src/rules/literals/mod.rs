...
//! one config option.
//!
//! [`LiteralRegistry::from_config`] resolves the active chain from `&Config` (skipping
//! `Preserve`); the [`LiteralRegistry`] applies them in turn, exactly as the prior hand-written
//! chain in `token_text` did. The three rules touch disjoint parts of a literal — the fraction, the hex
...
impl LiteralRegistry {
    /// Resolve the active rules from `cfg`, in the order `token_text` applied them. A rule whose
    /// option is `Preserve` is omitted, so the default config yields an empty chain (and literal
    /// emission stays verbatim).
