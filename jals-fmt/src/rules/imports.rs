...
//!
//! [`ImportRule::lower_source_file`] lowers the compilation unit; when an [`ImportOrdering`] is
//! configured, the contiguous leading run of `IMPORT_DECL`s is reordered before emission.
//! The pure planning step ([`ImportPlan::plan`]: pick the permutation and the group boundaries) is
...
impl ImportRule {
    ...
    /// [`Ctx::lower_items`]. Under [`ImportOrdering::Sort`] the leading run of `import` declarations
    ...
    pub(crate) async fn lower_source_file(node: &SyntaxNode, ctx: &Ctx<'_>) -> Doc {
        ...
        let block_lead_blanks = Self::import_block_lead_blanks(first, ctx);
        ...
        for (i, el) in els.iter().enumerate() {
                ...
                if saw {
                    parts.push(if !emitted_import {
                        ...
                        if block_lead_blanks > 0 {
                            Doc::blank_line(block_lead_blanks)
                        } else {
                            Doc::hardline()
                        }
                    } else if new_group[k] {
            ...
            if let Some(child) = el.as_node() {
                if Ctx::first_sig_token(child).is_none() {
                    continue;
                }
                if saw {
    ...
    /// The blank lines preceding the import block — the value [`Ctx::item_separator`] would compute
    /// for the originally-first import (its gap from the package decl or file start).
    fn import_block_lead_blanks(first_import: &SyntaxNode, ctx: &Ctx<'_>) -> usize {
        match Ctx::first_sig_token(first_import) {
            Some(t) if ctx.comments.has_leading(&t) => ctx.comments.blank_lines_before_first(&t),
            Some(t) => Ctx::blank_lines_before(&t),
            None => 0,
        }
    }
    ...
    fn import_sort_key(node: &SyntaxNode) -> (u8, String) {
        (Self::import_tier(node), Self::import_name(node))
    }

    /// The group rank of an import under `groups` (the `import-groups` list); lower ranks emit
    /// first. Module imports lead in their own group (rank `0`); every other import is the rank
    /// below shifted up by one. A non-static import takes the index of its *longest* matching prefix
    /// (ties broken by list order — the earliest such prefix wins); failing that, the catch-all
    /// `"*"`'s index, or `groups.len()` when the list has no `"*"`. A static import takes
    /// `"static"`'s index, or `groups.len() + 1` (after the catch-all) when the list has no
    /// `"static"`. The literals `"*"`, `"static"` and `"module"` are never matched as textual
    /// prefixes (they are keywords, so they can never be a real name segment).
    fn import_group_rank(node: &SyntaxNode, groups: &[String]) -> usize {
        if Self::import_tier(node) == 0 {
            return 0;
        }
        ...
        let name = Self::import_name(node);
...
impl ImportPlan {
    ...
    fn plan(mut run: Vec<SyntaxNode>, ordering: ImportOrdering<'_>) -> Self {
        match ordering {
            ImportOrdering::Group(groups) => {
                let mut keyed: Vec<(usize, String, SyntaxNode)> = run
                    ...
                    .map(|n| {
                        (
                            ImportRule::import_group_rank(&n, groups),
                            ImportRule::import_name(&n),
                            n,
                        )
                    })
                    .collect();
            ...
            ImportOrdering::Sort => {
                run.sort_by_cached_key(ImportRule::import_sort_key);
...
mod tests {
    ...
    // Thin shims so the tests reach the import planning / classification helpers, which now live as
    // associated functions on `ImportRule` / `ImportPlan`, without qualifying every call site.
    fn import_run(els: &[SyntaxElement]) -> Option<(usize, usize)> {
        ImportRule::import_run(els)
    }
    fn import_name(node: &SyntaxNode) -> String {
        ImportRule::import_name(node)
    }
    fn import_group_rank(node: &SyntaxNode, groups: &[String]) -> usize {
        ImportRule::import_group_rank(node, groups)
    }
    fn plan_run(run: Vec<SyntaxNode>, ordering: ImportOrdering<'_>) -> ImportPlan {
        ImportPlan::plan(run, ordering)
    }
