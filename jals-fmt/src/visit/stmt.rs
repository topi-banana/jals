...
impl Ctx<'_> {
    ...
    pub(super) async fn visit_switch_group(&mut self, node: &SyntaxNode) {
        ...
        for child in Self::children(node) {
            ...
            // statements: `visitStatements` asks for `BlankLineWanted.PRESERVE`. Not above the
            // first one, though — the label and what it introduces are one thing.
            let blanks = if opening {
                0
            } else {
                child.as_node().map_or(0, |statement| {
