...
use jals_hir::{Completion, DefKind, FileId, ProjectIndex, member_completions, resolve_node};
use jals_syntax::SyntaxNode;

/// Build an index over `sources`, place the cursor at the `$0` marker in `sources[file]` (removed
/// before parsing), and run member completion there.
fn complete(sources: &[&str], file: usize) -> Vec<Completion> {
    ...
    member_completions(root, &resolved, &index, *fid, offset)
}
