...
| [`jinja`](crates/jinja)              | [minijinja](https://docs.rs/minijinja) と同形の API を持つ汎用 Jinja2 テンプレートエンジン。依存ゼロで `jals` の型を一切名指ししません。`jals-project` が `[build.resources] template` の描画に使います。ワークスペース唯一の非 `jals-*` crate であり、`crates/` という置き場所がそれを表しています。                                                          |
