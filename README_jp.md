...
`jals` は 4 つのコア crate とブラウザ向け playground からなる Cargo ワークスペースです。
...
`jals` はサブコマンド方式で、`fmt`（ソース整形）と `lsp`（language server）があります。
...
カバーしていますが、API は変更される可能性があります。構文層の次の利用者としては linter
（`jals-lint`）を想定しています。
