...
- **`wasm32` 対応のコア。** エディタ座標変換・構文・フォーマット・lint・セマンティック解析の各層
  ...
  `jals-decompile` / `jals-fs` / `jals-config`）は `no_std` で `wasm32-unknown-unknown` 向けに
...
| Crate | 説明 |
| --- | --- |
| [`jals-editor`](jals-editor) | UTF-8 バイトオフセットと 0 始まり UTF-16 座標のエディタ非依存な変換。LSP とブラウザ playground で共有します。 |
...
```
jals/
├── jals-editor/      # UTF-8 byte <-> UTF-16 座標             (wasm 対応)
...
- `jals-editor` / `jals-syntax` / `jals-fmt` / `jals-lint` / `jals-hir` / `jals-classfile` /
  `jals-decompile` / `jals-fs` / `jals-config` は `no_std` crate として
