...
`jals.toml` マニフェストから JDK の `javac` / `java` をラップし、コンパイル前に sandbox 化された
Rhai build script も実行できる Cargo 風のビルドフロントエンド（`jals build` / `run` / `clean` /
`init`）も備えています。
...
- **`wasm32` 対応のコア。** エディタ座標変換・構文・フォーマット・lint・セマンティック解析の各層
  ...
  解決コアと `jals-build` の Rhai runner も同様です（ホスト I/O は `native` feature の背後に
  あります）。これによりブラウザ playground は同じ解析・build-script stack をクライアント側だけで
  動かせます。
...
読み、`target/jals/build/rhai/out` 以下だけへ file を生成し、生成 source・classpath entry・`javac`/JVM
flag・compile/run environment entry を追加できます。ホスト filesystem・process・network・clock・random
にはアクセスできません。CLI・LSP・browser playground は同じ portable engine を使い、LSP/playground
は JDK を実行せず、生成 source と classpath を解析へ取り込みます。完全な API、fingerprint/cache、
...
Rhai phase 自体は capability 制限されていますが、compiler/JVM 引数、classpath、subprocess environment
の directive は、後続の明示的な `jals build` / `run` による JDK process へ意図的に反映されます。
信頼していない checkout をビルドする前に、project code と同様に build script を確認してください。
...
の各層を、サーバを介さずブラウザ上だけで動かします。生成 Java source と `jals.toml` の
`[dependencies]` もブラウザ内で解決するため、hover / 補完 / 型検査がそれらを認識できます。
...
フロントエンドは、現状は portable な Rhai pre-build phase を備えた忠実ながら薄い `javac`/`java`
ラッパーであり、より本格的な依存関係管理・テスト・パッケージングは
