...
- **`wasm32` 対応のコア。** 構文・フォーマット・lint・セマンティック解析の各層（`jals-syntax` /
  `jals-fmt` / `jals-lint` / `jals-hir` / `jals-classfile` / `jals-decompile` / `jals-fs` /
  `jals-config`）は `no_std` で `wasm32-unknown-unknown` 向けにビルドでき、`jals-classpath` の
  解決コアも同様です（ホスト I/O は `native` feature の背後にあります）。これによりブラウザ
  playground は同じ解析スタックをクライアント側だけで動かせます。

## ワークスペース構成

`jals` は 13 個の crate とブラウザ向け playground からなる Cargo ワークスペースです。
...
| [`jals-lint`](jals-lint) | linter（`jals-cli` 経由の `jals lint`）。CST と `jals-hir` に基づくルールレジストリで、未使用のローカル変数・型不一致・報告されていない例外・定数条件による到達不能分岐・edition に応じたプレビュー機能チェックを行います。 |
...
| [`jals-fs`](jals-fs) | pure な crate 群が設定やソースを読み込むための仮想ファイルシステム trait（`FileTree`）。インメモリ実装と、`std` の背後にある実ファイルシステム実装を持ちます。 |
...
├── jals-fmt/         # フォーマッタ (CST -> Doc IR -> テキスト) (wasm 対応)
├── jals-lint/        # linter (CST + jals-hir 上のルール)      (wasm 対応)
├── jals-hir/         # 名前解決 + 型推論                        (wasm 対応)
├── jals-classfile/   # JVM .class 読み書きモデル                (wasm 対応)
├── jals-decompile/   # .class -> 読める Java                    (wasm 対応)
├── jals-classpath/   # classpath + 依存関係の解決               (wasm 対応コア)
├── jals-config/      # jals.toml/jalsfmt.toml/jalslint.toml モデル (wasm 対応)
├── jals-fs/          # 仮想ファイルシステム抽象                  (wasm 対応)
...
分岐、edition に応じたプレビュー機能を、名前解決と型推論（`jals-hir`）を使って検出します。単なる
...
- `jals-syntax` / `jals-fmt` / `jals-lint` / `jals-hir` / `jals-classfile` / `jals-decompile` /
  `jals-fs` / `jals-config` は `no_std` crate として `wasm32-unknown-unknown` 向けにビルドできる。
