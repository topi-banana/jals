...
4. **Formatter fidelity.** Comments are never dropped, and formatting is idempotent
   ...
   tokens) is preserved exactly. Two options, each off by default, relax this:
   ...
   Idempotency holds in every case. With both options at their defaults (`reorder-imports` off,
   `trailing-comma = preserve`), the exact-sequence guarantee is in full force.
