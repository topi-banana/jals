...
| Playground | `jals-playground/` | Yew (CSR) browser app served by Trunk (`Trunk.toml`, tailwind); compiles to `wasm32`. Runs the syntax/formatter/analysis in-browser. **External `[dependencies]`** work in the browser: a `DepsBar` accepts a `jals.toml` (parsed via `Manifest::from_str`), a `BrowserFetcher` (`fetcher.rs`, `gloo-net` `fetch`, optional CORS proxy) downloads each jar into an `InMemoryFileTree` cache, and `jals_classpath::{resolve_project_dependencies_in,load_classpath_in}` + `ProjectIndex::lower_classpath` fold the classpath into `Workspace`'s index (`with_classpath`), so hover/completion/type-check resolve library types. CORS limits direct fetch to permissive hosts (Maven Central needs the proxy). |
