...
- Crate graph: `jals-cli` → `{jals-fmt, jals-lint, jals-lsp, jals-build, jals-hir, jals-classfile, jals-syntax}`;
  `jals-lsp` → `{jals-fmt, jals-lint, jals-hir, jals-syntax}`; `jals-lint` → `{jals-hir, jals-syntax}`;
  `jals-hir` → `{jals-syntax, jals-classfile}`; `jals-fmt` → `jals-syntax`.
...
5. **`wasm32` compatibility.** Everything except `jals-cli` and `jals-lsp` must build for
   ...
   usage to `jals-syntax`, `jals-fmt`, `jals-build`, `jals-hir`, or `jals-classfile`; keep that work
   in `jals-cli`/`jals-lsp` (`jals-build` only *plans* `javac`/`java` commands as pure data —
   `jals-cli` spawns them; `jals-classfile` is a pure byte-level codec — `jals-cli` reads the bytes).
