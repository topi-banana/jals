...
5. **`no_std` + `wasm32` compatibility.** The seven pure crates — `jals-syntax`, `jals-classfile`,
   ...
   fs access through `jals-fs`); keep host work in `jals-cli`/`jals-lsp` and `jals-classpath`'s
   `native` feature. CI builds all seven pure crates for `wasm32` in the `build` matrix (as one
   ...
   **`jals-classpath` (with `--no-default-features`) is also `wasm32`-built** in a separate matrix
   ...
   stays out of the seven-crate `no_std` set, like `jals-playground`. When editing `jals-classpath`,
