...
4. **Formatter fidelity.** Comments are never dropped, and formatting is idempotent
   ...
   tokens) is preserved exactly. Five options, each off by default, relax this:
   ...
   Idempotency holds in every case. With all five at their defaults (`reorder-imports`,
   `group-imports`, and `reorder-modifiers` off, `trailing-comma = preserve`,
   `hex-literal-case = preserve`), the exact-sequence guarantee is in full force.
