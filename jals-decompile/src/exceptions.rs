...
pub(crate) use api::regions;

/// Recovering `try` statements from an exception table.
mod api {
    use super::{Cfg, Clause, Finally, Group, Instruction, JvmKind, TryRegion, Vec};
    use crate::body;
    ...
    pub(crate) fn regions(cfg: &Cfg, code: &[Instruction]) -> Option<Vec<TryRegion>> {
        ...
        let groups = clauses(cfg, code)?;
        let mut regions = statements(cfg, code, groups)?;
    ...
    fn clauses(cfg: &Cfg, code: &[Instruction]) -> Option<Vec<Group>> {
        ...
        for entry in &cfg.handlers {
            ...
            let Some(index) = known else {
                groups.push(Group {
                    handler: entry.entry,
                    param_pc: param_pc(cfg, entry.entry)?,
                    slot: entry_slot(cfg, code, entry.entry)?,
    ...
    fn statements(cfg: &Cfg, code: &[Instruction], groups: Vec<Group>) -> Option<Vec<TryRegion>> {
        ...
        for group in finals {
            ...
            region.finally = Some(finalizer(cfg, code, &group)?);
        }
        Some(regions)
    }
    ...
    fn finalizer(cfg: &Cfg, code: &[Instruction], group: &Group) -> Option<Finally> {
        ...
        if body_end < body_start || body::load_slot(code.get(body_end)?) != Some(group.slot) {
            return None;
        }
        ...
        if body.is_empty() || !foldable(&code[body.clone()]) {
            return None;
        }
