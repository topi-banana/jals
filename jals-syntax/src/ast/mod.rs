...
mod ast_support {
    use super::{IDENT, String, SyntaxNode, SyntaxToken};

    /// Returns the first significant token (non-trivia) of `node`, if any.
    pub(super) fn first_sig_token(node: &SyntaxNode) -> Option<SyntaxToken> {
    ...
    #[cfg(test)]
    pub(super) fn non_trivia_text(node: &SyntaxNode) -> String {
    ...
    /// The text of [`name_token`](name_token), with its JLS §3.3 escapes resolved. Defined in
    ...
    pub(super) fn name_text(node: &SyntaxNode) -> Option<String> {
        name_token(node).map(|t| crate::decoded_ident(&t).into_owned())
    }
    ...
    pub(super) fn ident_tokens(node: &SyntaxNode) -> impl Iterator<Item = SyntaxToken> {
        node.children_with_tokens()
            .filter_map(rowan::NodeOrToken::into_token)
