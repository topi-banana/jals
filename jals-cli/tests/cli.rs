...
fn resources_are_packaged_into_the_jar() {
    ...
    // The two routes are independent and both run. The remap jar takes its copy from the snapshot,
    // above; the Maven-style copy step puts the same file in `classes-dir`, which is what `jals run`
    // executes and what `jals package` archives. A resource reaches a jar by whichever route the
    // command that built it uses, and neither reads the other's output.
