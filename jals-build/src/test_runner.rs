...
//!   prints a sentinel line, and only that line is a pass.
...
/// Where a test run's per-process scratch lives, relative to the project root.
///
...
/// The two strings the generated harness and this runner have to agree on.
///
...
/// both from there and hands them over without interpreting either.
#[derive(Debug, Clone)]
...
impl TestLauncher {
    ...
    pub async fn resolve(
        ...
        let scratch_root = request.project_root.join(
    ...
    /// Clear the previous run's scratch so a stale capture can never be read as this run's.
    ...
    ///
    /// # Errors
    /// [`ToolchainError::Spawn`] when the JVM cannot be started, and
    /// [`ToolchainError::ArgumentFile`] when its output cannot be read.
    ...
    ///
    /// Polls with a bounded backoff rather than blocking in `wait()`: the deadline has to be
    /// observable, and this runs on a fan-out worker thread where sleeping costs nothing the
    /// executor can feel.
...
impl SharedRun {
    ...
    fn judge(&self, raw: &RawOutcome, case: &TestCase) -> TestVerdict {
        ...
        let passed = std::fs::read_to_string(&raw.stdout)
            .is_ok_and(|text| text.lines().any(|line| line.trim_end() == sentinel));
        if passed {
            TestVerdict::Passed
        } else {
            TestVerdict::Failed { code: Some(code) }
