...
set is different: it is always an input, whether or not the script reads it, so a changed
`--features` selection can never reuse a build made under a different one. Managed build-output paths
