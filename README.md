...
- **Java packages written in Rust.** A `native` method compiles to a WebAssembly import, and a
  **native package** is one Rust crate holding both halves of that declaration: the Java it
  ...
  the import's name *is* the descriptor. `jals` ships two: `jals.io`, which makes
  `examples/hello_world_native` print `Hello, world!` from a module that has no `String` in it,
  and **`java.base`** — the `java.lang` and `java.io` that a wasm host does not have, as fifty-two
  Java files behind ten host functions, so `examples/hello_world_java_base` prints the same
  greeting through `System.out.println(new String(…))`.
...
| [`jals-native`](jals-native)         | A Java package whose implementation is Rust: the Java it publishes and the host functions its `native` methods bind to, in one value. `[build] native-packages` selects one; the wasm backend turns each `native` method into an import and the runner links it. Ships `jals.io` and `java.base` — the latter being `java.lang` and `java.io` with the bodies `jals-hir`'s stubs do not have.                                                                                                                                                                                                                       |
