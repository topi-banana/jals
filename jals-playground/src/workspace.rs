...
//! The live buffer is reflected with [`sync_active`](Workspace::sync_active) — a no-op when the
//! text is unchanged, so a hover storm over an idle buffer never re-analyzes.
...
impl Workspace {
    ...
    pub fn analyze_active(&self, config: &LintConfig) -> Vec<PlaygroundDiagnostic> {
        self.editor.diagnostics(&self.active, config)
    }
    ...
    pub fn hover(&self, line: u32, col: u32) -> Option<String> {
        self.editor.hover(&self.active, &(line, col))
    }
    ...
    pub fn completions(&self, line: u32, col: u32) -> Vec<CompletionEntry> {
        self.editor.completions(&self.active, &(line, col))
    }
    ...
    pub fn signature_help(&self, line: u32, col: u32) -> Option<SigHelp> {
        self.editor.signature_help(&self.active, &(line, col))
    }
    ...
    pub fn document_highlight(&self, line: u32, col: u32) -> Vec<Highlight> {
        self.editor.highlights(&self.active, &(line, col))
    }
    ...
    pub fn goto_definition(&self, line: u32, col: u32) -> Option<Target> {
        self.editor.definition(&self.active, &(line, col))
    }
    ...
    pub fn references(&self, line: u32, col: u32, include_declaration: bool) -> Vec<Target> {
        self.editor
...
mod tests {
    ...
    fn sync_active_no_op_keeps_the_cached_parse() {
        ...
        // Unchanged text: the overlay (and its parse) is untouched — a hover storm re-analyzes
        // nothing.
        ws.sync_active(&src);
        assert_eq!(
    ...
    fn goto_definition_navigates_cross_file() {
        ...
        let target = ws.goto_definition(line, col).expect("type resolves");
        assert_eq!(target.path, "com/example/Greeter.java");
        // The range maps against the *target* file's text: `class Greeter` sits on its line 2.
    ...
    fn references_span_the_workspace() {
        ...
        let without_decl = ws.references(line, col, false);
        ...
        let with_decl = ws.references(line, col, true);
    ...
    fn completions_after_dot_list_members_without_keywords() {
        ...
        let entries = ws.completions(line, col);
        assert!(
            entries
                .iter()
    ...
    fn signature_help_marks_the_active_parameter() {
        ...
        let help = ws.signature_help(line, col).expect("inside a call");
        assert_eq!(help.signatures.len(), 1);
        assert_eq!(help.signatures[0].label, "area(int w, int h)");
        // The parameter spans are UTF-16 offsets into the label: `int h` follows `area(int w, `.
    ...
    fn document_highlight_covers_a_local() {
        ...
        let highlights = ws.document_highlight(line, col);
