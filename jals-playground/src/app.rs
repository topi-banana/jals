...
impl App {
    ...
    async fn resolve_classpath(
        ...
        // Every warning rendered whole, through its own `Display`: the status line is this host's
        // only channel, and neither a graph warning that does not name its dependency nor a lowering
        // warning that does not name the `[build]` entry it came from is actionable in a browser.
