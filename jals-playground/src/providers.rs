...
pub fn install(workspace: Rc<RefCell<Workspace>>) {
    install_hover(workspace.clone());
    install_completion(workspace.clone());
    install_signature_help(workspace.clone());
    install_document_symbols(workspace.clone());
    install_document_highlight(workspace.clone());
    install_definition(workspace.clone());
    install_references(workspace);
}

/// Hand `closure` to `register_with` (the matching `monaco::register_*`) and leak it, so the once-
/// registered provider stays live for the app's single editor. Centralises the easy-to-drop
/// `forget()` every installer would otherwise repeat.
...
/// Install a position provider: a `(&Workspace, text, line, col) -> JsValue` body backed by `ws`,
/// wired to its matching `monaco::register_*`. Folds the shared `Closure` construction, immutable
/// `borrow`, and `register`/`forget` that the five position-based installers would otherwise repeat.
fn install_pos(
...
/// Marshal a navigation [`Target`] into a Monaco `Location` payload (shared by go-to-definition and
/// find-references).
        ...
        |ws, text, line, col| {
            ...
            for entry in ws.completions(text, line, col) {
                let kind = if entry.keyword {
                    ...
                    monaco::completion_kind(entry.kind)
                };
