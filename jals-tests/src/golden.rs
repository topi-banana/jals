...
pub const PALANTIR: Target = Target {
    name: "palantir",
...
pub const ECLIPSE: Target = Target {
    name: "eclipse",
    tool: "Eclipse JDT",
    tier: Tier::T2,
...
pub const INTELLIJ: Target = Target {
    name: "intellij",
    tool: "IntelliJ IDEA",
    tier: Tier::T2,
...
impl GoldenSource {
    ...
    /// `tool version` — the reference this corpus's `.output` files are defined by.
    pub fn reference(&self) -> String {
        format!("{} {}", self.target.tool, self.pin.version())
    }
}
...
impl GoldenReport {
    ...
    pub fn run(name: &str, root: &Path, target: &Target, pin: Pin) -> Self {
        ...
        Self {
            name: name.to_string(),
            reference: format!("{} {}", target.tool, pin.version()),
            tier: target.tier,
