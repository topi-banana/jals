...
This file exists so the directory does too. `[build] source-dirs` names it, a publication
destination has to be a strict descendant of a declared source root, and a project that declares a
source root it does not have is reported as such by anyone depending on it — which, before the first
build has published anything, is exactly this directory.
