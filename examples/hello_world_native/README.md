...
`jals` ships the platform and `jals.io`; the browser playground ships the same set over its own
host; a program embedding this toolchain registers whatever it likes. A name none of them offers is
reported by the host that holds the registry, with the names it does offer.

`jals-platform`'s `java.base` is selected by `[build] platform`, which defaults to it; `jals.io` is
named in `[build] native-packages`, which is the route a third-party package arrives by. The two
keys are different questions: the platform is what `java.lang` *is* on this target, and a native
package is something a project opts into.
