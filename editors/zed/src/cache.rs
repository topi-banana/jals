...
use crate::{platform, GITHUB_REPO};
...
pub(crate) use api::{fetch_artifact, is_file, newest_cached_binary};
...
mod api {
    ...
    pub(crate) fn fetch_artifact(
        ...
        if !is_file(&binary_path) {
            ...
            remove_stale_version_dirs(&version_dir);
        }
    ...
    pub(crate) fn version_dirs() -> impl Iterator<Item = (String, u64)> {
        fs::read_dir(".")
            .into_iter()
            .flatten()
            .flatten()
    ...
    pub(crate) fn remove_stale_version_dirs(current_dir: &str) {
        for (dir_name, _) in version_dirs() {
