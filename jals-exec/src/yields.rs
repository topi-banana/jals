...
/// Yields once: wakes itself and returns `Pending` a single time, sending the task to the back
/// of the current executor's queue.
...
///
/// Correct for the workspace's ready-poll futures (inline yields make progress on every poll);
/// a future waiting on a genuinely external event would spin. Hosts have real runtimes — this
/// exists for tests and for the inline executor itself.
