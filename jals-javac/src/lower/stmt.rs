...
pub(crate) use api::{block, discarded, lower};
...
mod api {
    ...
    pub(crate) fn block(
        ...
        // A local type declaration is not a statement to emit: `lower::file` compiles it as its own
        // class file, the same way it does every other declaration in the file.
        for statement in block.stmts() {
            lower(&statement, context, emit)?;
        }
        Ok(())
    }
    ...
    pub(crate) fn lower(
        ...
        labelled(statement, Vec::new(), context, emit)
    }
    ...
    pub(crate) fn labelled(
        ...
        match statement {
            ast::Stmt::Labeled(outer) => {
                ...
                labelled(&inner, labels, context, emit)
            }
            ast::Stmt::While(statement) => while_loop(statement, labels, context, emit),
            ast::Stmt::DoWhile(statement) => do_while(statement, labels, context, emit),
            ast::Stmt::For(statement) => for_loop(statement, labels, context, emit),
            ast::Stmt::ForEach(statement) => for_each(statement, labels, context, emit),
            ...
            other if !labels.is_empty() => labelled_block(other, labels, context, emit),
            ast::Stmt::Block(block) => self::block(block, context, emit),
            ast::Stmt::Empty(_) => Ok(()),
            ast::Stmt::LocalVar(declaration) => local(declaration, context, emit),
            ast::Stmt::Expr(expression) => self::expression(expression, context, emit),
            ast::Stmt::Return(statement) => ret(statement, context, emit),
            ast::Stmt::If(statement) => conditional(statement, context, emit),
            ast::Stmt::Break(statement) => leave(statement.label(), true, context, emit),
            ast::Stmt::Continue(statement) => leave(statement.label(), false, context, emit),
            ast::Stmt::Throw(statement) => throw(statement, context, emit),
            ast::Stmt::Synchronized(statement) => synchronized(statement, context, emit),
            ast::Stmt::Try(statement) => try_catch(statement, context, emit),
            ast::Stmt::Assert(statement) => assert(statement, context, emit),
            ast::Stmt::Switch(statement) => {
                crate::lower::switch::statement(statement, labels, context, emit)
            }
            ...
            ast::Stmt::Yield(statement) => yield_value(statement, context, emit),
        }
    }
    ...
    pub(crate) fn yield_value(
        ...
        if !crossed.is_empty() {
            ...
            emit.asm.store(held)?;
            run_guards(&crossed, context, emit)?;
    ...
    pub(crate) fn cleanup(
        ...
        match cleanup {
            ...
            Cleanup::Close {
                ...
                owner,
                interface,
            } => close(*slot, owner, *interface, emit),
        }
    }
    ...
    pub(crate) fn try_catch(
        ...
        resource_chain(&resources, &body, context, emit)?;
        let end = emit.asm.label();
        emit.asm.mark(end)?;
        ...
        for (clause, &handler) in clauses.iter().zip(&handlers) {
            for ty in clause.types() {
                ...
                let entry = desc::class_entry(&caught, context.index)?;
        ...
        for (clause, &handler) in clauses.iter().zip(&handlers) {
            ...
            let caught = caught_type(clause, context)?;
            emit.asm
                .bind_handler(handler, start, &desc::class_entry(&caught, context.index)?)?;
            if let Some(cleanup) = &cleanup {
    ...
    pub(crate) fn resource_chain(
        ...
        let Some((first, rest)) = resources.split_first() else {
            return block(body, context, emit);
        };
        let (slot, owner, interface) = acquire(first, context, emit)?;
        ...
        resource_chain(rest, body, context, emit)?;
        let end = emit.asm.label();
        emit.asm.mark(end)?;
        let ranges = emit.unguard(end);
        if emit.asm.reachable() {
        ...
        close_suppressing(slot, &owner, interface, primary, emit)?;
    ...
    pub(crate) fn acquire(
        ...
        let owner = desc::internal_name_of(declaring, context.index);
    ...
    pub(crate) fn expression(
        ...
        discarded(&expression, context, emit)
    }
    ...
    pub(crate) fn labelled_block(
        ...
        let done = emit.asm.label();
        in_scope(labels, done, None, emit, |emit| {
            lower(statement, context, emit)
        })?;
        join(done, emit)
    }
    ...
    pub(crate) fn while_loop(
        ...
        in_scope(labels, done, Some(test), emit, |emit| {
            body(statement.body().as_ref(), context, emit)
        })?;
    ...
    pub(crate) fn for_loop(
        ...
        for section in statement.init() {
            for_section(&section, context, emit)?;
        }
        ...
        in_scope(labels, done, Some(next), emit, |emit| {
    ...
    pub(crate) fn for_section(
        ...
        if let Some(declaration) = ast::LocalVarDecl::cast(node.clone()) {
            return local(&declaration, context, emit);
        }
    ...
    pub(crate) fn for_each(
        ...
        let Ty::Array(element) = expr::type_of(iterable.syntax(), context)? else {
            return for_each_iterator(statement, &iterable, labels, context, emit);
        };
        ...
        let binding = for_each_binding(statement, context)?;
        let variable = emit
            .slots
        ...
        let declared =
            desc::descriptor_of(&context.typed.type_of_def(binding).clone(), context.index)?
                .to_string();
    ...
    pub(crate) fn for_each_iterator(
        ...
        emit.asm
            .check_cast(&desc::class_entry(&element, context.index)?)?;
        let declared = desc::descriptor_of(&element, context.index)?.to_string();
        emit.asm.store_as(variable, &declared)?;
    ...
    pub(crate) fn body(
        ...
        body.map_or(Ok(()), |body| lower(body, context, emit))
    }
