//! The Rust half: the host functions the platform's Java declares, and why it is exactly these.
//!
...
use jals_native::{Args, JavaPackage, NativeError, NativeHost, NativeValue, RefSlot, Results};
...
impl Bindings {
    ...
    ///
    /// The groups are separate functions because they need different things: the streams need
    ...
    pub fn install(package: &mut JavaPackage, host: &Rc<dyn PlatformHost>) {
        ...
        crate::collections::Collections::install(package);
    }
    ...
    fn bind_streams(package: &mut JavaPackage, host: &Rc<dyn PlatformHost>) {
        ...
        package.bind(
            ...
            move |_, args, _| {
                ...
                // Lossy, and the loss is a real one: an unpaired surrogate becomes U+FFFD. That is
                // what a program wrote, though — the pairing happened or it did not, and the
                // buffering above is what makes a *paired* one arrive whole.
