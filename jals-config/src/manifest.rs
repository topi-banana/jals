...
pub struct Build {
    ...
    /// Resource roots, relative to the manifest directory. Defaults to `["src/main/resources"]`.
    ///
    ...
    ///
    /// They reach output by two routes, and both start here. A host copies them into
    /// [`classes_dir`](Build::classes_dir) after a successful compile, keeping each path below its
    /// root — Maven's `src/main/resources` step — so `jals run` puts them on the classpath and
    /// `jals package` archives them for the same reason it archives a class. The jar
    /// [`remap`](Build::remap) writes packages them directly from the snapshot instead, because
    /// remapping runs where no host filesystem is in reach; the browser playground refuses a
    /// declared `remap` outright and so sees neither route.
