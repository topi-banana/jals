...
impl Codegen {
    ...
    pub(crate) fn run(check: bool) -> Result<()> {
        ...
        // The platform package.s source list is generated by the same command, so one gate covers
        // both: a `.java` nobody listed and a grammar change nobody regenerated fail together.
        crate::platform::Platform::run(&root, check)?;
        if check {
    ...
    fn generate(grammar: &Grammar) -> Result<String> {
        let items = Item::lower(grammar)?;
        // The items are rendered before the header so the header can be written against what was
        // actually emitted. An unused import warns inside a do-not-edit file, so the alternative is
        // a second list saying which accessor kinds name which type — a rule that would have to be
        // updated alongside every emitter, and so would eventually not be.
        let mut body = String::new();
        for item in &items {
            match item {
                Item::Node(node) => node.render(&mut body),
                Item::Enum(enm) => enm.render(&mut body),
            }
        }
...
impl Item {
    /// Lowers every grammar rule to a generated item, in grammar order.
    fn lower(grammar: &Grammar) -> Result<Vec<Item>> {
        ...
        for node in grammar.iter() {
            ...
            if let Some(node_names) = EnumSrc::node_alternatives(grammar, &data.rule) {
                let variants: Vec<_> = node_names
                    .into_iter()
                    .map(|node_name| {
                        let kind = screaming_snake(&node_name);
                        (EnumSrc::variant_name(&name, &node_name), node_name, kind)
                    })
                    .collect();
                ...
                for (i, (variant, node, _)) in variants.iter().enumerate() {
                    if let Some((_, earlier, _)) =
                        variants[..i].iter().find(|(v, _, _)| v == variant)
                    {
                        bail!(
                ...
                Accessor::collect(grammar, &name, &data.rule, &mut accessors)?;
                Accessor::check_conflicts(&name, &accessors)?;
...
impl EnumSrc {
    ...
    fn node_alternatives(grammar: &Grammar, rule: &Rule) -> Option<Vec<String>> {
    ...
    fn render(&self, buf: &mut String) {
        ...
        for (variant, node, _) in &self.variants {
...
impl Accessor {
    ...
    fn collect(grammar: &Grammar, owner: &str, rule: &Rule, out: &mut Vec<Accessor>) -> Result<()> {
        match rule {
            Rule::Labeled { label, rule } => {
                out.push(Accessor::from_labeled(grammar, owner, label, rule)?);
            }
            Rule::Node(_) | Rule::Token(_) => {}
            Rule::Seq(rules) | Rule::Alt(rules) => {
                for rule in rules {
                    Accessor::collect(grammar, owner, rule, out)?;
                }
            }
            Rule::Opt(rule) | Rule::Rep(rule) => Accessor::collect(grammar, owner, rule, out)?,
        }
        Ok(())
    }

    fn from_labeled(grammar: &Grammar, owner: &str, label: &str, rule: &Rule) -> Result<Accessor> {
        let mut shape = Shape::default();
        shape.extend(grammar, rule);
        ensure!(
            !shape.has_label,
        ...
        let kind = if shape.nodes.is_empty() {
            ...
            if label.starts_with("is_") {
                let kind = AccessorKind::token_kind(token).with_context(|| {
...
impl Shape {
    /// Accumulates every node/token reference in `rule` into this shape.
    fn extend(&mut self, grammar: &Grammar, rule: &Rule) {
        match rule {
            Rule::Labeled { .. } => self.has_label = true,
            Rule::Node(node) => self.nodes.push(grammar[*node].name.clone()),
            Rule::Token(token) => self.tokens.push(grammar[*token].name.clone()),
            Rule::Seq(rules) | Rule::Alt(rules) => {
                for rule in rules {
                    self.extend(grammar, rule);
                }
            }
            Rule::Opt(rule) => self.extend(grammar, rule),
            Rule::Rep(rule) => {
                self.has_rep = true;
                self.extend(grammar, rule);
            }
...
mod tests {
    ...
    #[test]
    fn grammar_lowers() {
        let items = Item::lower(&grammar()).unwrap();
