...
// the chain, `GraphError::InvalidDependency` the declaring node, and `ProjectAssemblyError` the
// node that failed. A host reads none of them: every one of these renders through its `Display`,
