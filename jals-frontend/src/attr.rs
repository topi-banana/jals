...
//!
//! [`AttrPlan::compute`] walks a parsed file and decides, per attribute host (an import, a
//! declaration, or a statement), what the dialect rewrite must do: blank the attribute text
//! itself (an enabled host — `javac` must never see `#[`), or blank the host's whole span (a
//! `cfg` predicate evaluated false against the resolved build features). Blanking is
//! length-preserving, so every other byte offset in the file — and every line number — stays
...
impl AttrPlan {
    ...
    pub(crate) fn compute(parse: &Parse, text: &str, features: &BTreeSet<String>) -> Self {
        ...
        // a for-init, a late attribute) and is diagnosed individually.
        ...
        while let Some(node) = stack.pop() {
            if node.kind() == SyntaxKind::ATTRIBUTE {
                ...
                if !handled.contains(&usize::from(node.text_range().start())) {
                    ...
                    let line =
                        Self::node_span(&node).map_or(1, |(start, _)| Self::line_of(text, start));
                    let late = node
                        .parent()
                        .is_some_and(|p| p.kind() == SyntaxKind::MODIFIERS)
                        && Self::has_preceding_significant(&node);
                    out.errors.push(if late {
                        Self::late_attribute(line)
                    } else {
                        Self::unsupported_position(line)
                    });
                }
                continue;
            }
            if Self::is_attribute_host(node.kind()) {
                let attrs = Self::leading_attributes(&node);
                if !attrs.is_empty() {
                    for attr in &attrs {
                        handled.insert(usize::from(attr.syntax().text_range().start()));
                    }
                    if out.host_plan(parse, text, &node, &attrs, features) == Host::Disabled {
                        continue;
                    }
                }
            }
            ...
            .filter(|token| token.kind() == SyntaxKind::HASH)
        {
            ...
            out.errors
                .push(Self::stray_hash(Self::line_of(text, offset)));
        }
        out
    }
    ...
    /// attribute blanks (enabled) or the whole-host blank (disabled). Every attribute on the host
    ...
    fn host_plan(
        ...
        for attr in attrs {
            let Some(span) = Self::node_span(attr.syntax()) else {
                self.errors.push(Self::malformed_cfg(1));
                continue;
            };
            let line = Self::line_of(text, span.0);
            if Self::overlaps_error(parse, span) {
                self.errors.push(Self::attr_overlaps_error(line));
                continue;
            }
            match Self::eval_attribute(attr, features, line) {
        ...
        if enabled {
            for attr in attrs {
                if let Some((start, end)) = Self::node_span(attr.syntax()) {
        ...
        let Some((start, end)) = Self::node_span(host) else {
            return Host::Enabled;
        };
        // Error recovery can mis-extend a node, so a disabled host overlapping a parse error is
        // never best-effort stripped.
        if Self::overlaps_error(parse, (start, end)) {
            self.errors
                .push(Self::disabled_has_errors(Self::line_of(text, start)));
            return Host::Enabled;
        }
        self.blanks.push(Blank {
            start,
            end,
            semicolon: Self::needs_semicolon(host),
        });
    ...
    /// statements (whose leading attributes are direct children); `BLOCK` counts because a block
    /// is a statement. Everything else — parameters, record components, catch/resource/for-header
    /// modifiers, enum constants — is rejected by the dangling-attribute check in
    ...
    /// exactly what keeps blanking line-stable.
    ...
    /// Whether any parse error lies strictly inside `span` (half-open, matching the
    /// grouped-import overlap test).
    ...
    fn eval_attribute(
        ...
        let Some(meta) = attr.meta() else {
            return Err(Self::malformed_cfg(line));
        };
        ...
        Self::eval_predicate(&predicate, features, line, 0)
    }
        ...
        match name.as_str() {
            "feature" => {
                ...
                let Some(value) = meta.value() else {
                    return Err(Self::non_string_feature(line));
                };
                Ok(features.contains(&Self::feature_name(&value, line)?))
            }
            "all" | "any" | "not" => {
                if meta.value().is_some() {
                    return Err(Self::malformed_cfg(line));
                ...
                match name.as_str() {
                    "all" => nested.try_fold(true, |acc, p| {
                        Ok(acc & Self::eval_predicate(&p, features, line, depth + 1)?)
                    }),
                    ...
                    _ => {
                        ...
                        Ok(!Self::eval_predicate(&inner, features, line, depth + 1)?)
                    }
                }
            }
            _ => Err(Self::malformed_cfg(line)),
        }
    }
    ...
    fn feature_name(value: &Literal, line: usize) -> Result<String, String> {
        ...
        let inner = text
            ...
            .ok_or_else(|| Self::non_string_feature(line))?;
