...
pub(crate) const RULE: RuleMeta = RuleMeta {
    ...
    check: Checker::Versioned(ModuleImport::check),
};
        ...
        file.imports()
            ...
            .map(|import| {
                Finding::at_node(
                    import.syntax(),
                    format!(
                        "module import declarations (`import module …;`) are a preview feature \
                         before Java {STABLE_VERSION}; this project targets Java {version}, so \
                         import the individual types or set `edition = \"java{STABLE_VERSION}\"`",
                    ),
                )
            })
            .collect()
