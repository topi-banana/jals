...
//!
//! This is the lint-side adapter over [`jals_hir::dead_ifs`], which does the whole analysis (see
//! there): it folds boolean / integer literals, parentheses, `!`, the short-circuit operators,
...
fn check(root: &SyntaxNode, resolved: &Resolved) -> Vec<Finding> {
    jals_hir::dead_ifs(root, resolved)
        .into_iter()
        .map(|d| Finding {
            message: d.message(),
