...
mod api {
    use super::{
        ASSERT_STMT, BLOCK, BREAK_STMT, BracePolicy, CONTINUE_STMT, Config, DO_WHILE_STMT,
        EMPTY_STMT, EXPR_STMT, FOR_EACH_STMT, FOR_STMT, Finding, IF_STMT, LABELED_STMT,
        LOCAL_VAR_DECL, LocalBoxFuture, RETURN_STMT, SWITCH_STMT, SYNCHRONIZED_STMT, SyntaxKind,
        SyntaxNode, THROW_STMT, TRY_STMT, Vec, WHILE_STMT, YIELD_STMT, Yielder, format,
        significant,
    };
    ...
    pub(crate) async fn check_impl(root: &SyntaxNode, config: &Config) -> Vec<Finding> {
        ...
        for node in root.descendants() {
            yielder.tick().await;
            match node.kind() {
                IF_STMT => check_if(&node, policy, &mut out),
                WHILE_STMT | FOR_STMT | FOR_EACH_STMT | DO_WHILE_STMT => {
                    ...
                    if let Some(body) = node.children().filter(|c| is_stmt(c.kind())).last()
                        && body.kind() != BLOCK
                        && requires_braces(&node, &body, policy)
                    {
    ...
    pub(crate) fn check_if(node: &SyntaxNode, policy: BracePolicy, out: &mut Vec<Finding>) {
        ...
        for (i, branch) in branches.iter().enumerate() {
            ...
            if !requires_braces(node, branch, policy) {
                continue;
            }
